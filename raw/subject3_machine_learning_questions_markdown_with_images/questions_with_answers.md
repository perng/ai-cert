# AI 應用規劃師（中級）科目 3：機器學習技術與應用 — 練習題擷取（含圖片）

Source: `AI應用規劃師(中級)-學習指引-科目3機器學習技術與應用_20251222101907.pdf`

Extracted practice questions: 40 questions total, 10 questions per chapter.

This version includes extracted PDF image assets and rendered source-page images for the extracted question/answer sections.

## 第三章 機器學習基礎數學

### Source page images

![PDF source page 35](images/source_pages/page_035.png)

![PDF source page 36](images/source_pages/page_036.png)

![PDF source page 37](images/source_pages/page_037.png)

![PDF source page 38](images/source_pages/page_038.png)

### Embedded PDF images found on these source pages

![Embedded image from PDF page 35](images/embedded/page_035_xref_711_171x43.png)

![Embedded image from PDF page 35](images/embedded/page_035_xref_713_161x43.png)

![Embedded image from PDF page 35](images/embedded/page_035_xref_716_49x50.png)

![Embedded image from PDF page 37](images/embedded/page_037_xref_717_51x52.png)

![Embedded image from PDF page 37](images/embedded/page_037_xref_720_38x38.png)

![Embedded image from PDF page 37](images/embedded/page_037_xref_730_52x52.png)
...
**解析：** 機率密度函數（PDF）用於描述連續型隨機變數於某個取值範圍內的機率分佈。與離散型隨機變數不同，連續變數在單一點的機率為零，PDF 通常透過積分計算區間內的機率，而PMF 則用於離散型變數。累積分佈函數（CDF） 則是累計機率的工具，而非直接描述機率密度。
...
**答案：C**
**答案內容：特徵值分解（Eigenvalue Decomposition）**
...
![PDF source page 131](images/source_pages/page_131.png)

![PDF source page 132](images/source_pages/page_132.png)

![PDF source page 133](images/source_pages/page_133.png)

![PDF source page 134](images/source_pages/page_134.png)
...
**答案：C**
**答案內容：非監督式學習（Unsupervised Learning）**
...
**答案：C**
**答案內容：卷積神經網路（Convolutional Neural Network, CNN）**
...
**答案：D**
**答案內容：Support Vector Regression**

**解析：** 當支援向量機（SVM）應用於連續數值的預測問題時，稱為支援向量迴歸（Support Vector Regression, SVR）。SVR 與傳統SVM 不同，目標是尋找一條最佳超平面，使大多數資料點落在預設的誤差範圍內。
...
![PDF source page 187](images/source_pages/page_187.png)

![PDF source page 188](images/source_pages/page_188.png)

![PDF source page 189](images/source_pages/page_189.png)

![PDF source page 190](images/source_pages/page_190.png)

![PDF source page 191](images/source_pages/page_191.png)
...
**答案：C**
**答案內容：預測模型填補（Predictive Imputation）**
...
**解析：** 四分位距法是常見的統計方法，透過計算資料的第一與第三四分位數 （Q1 與Q3），並判定距離超過IQR（四分位距）1.5 倍以上的數據點為異常。 這種方法直觀且適用於大多數數值型資料的初步檢查。
...
**解析：** LOOCV（Leave-One-Out Cross-Validation）是K-fold 的極端形式，當K 等於樣本數n 時，每次只留一筆做驗證。此方法能最大化資料利用，但計算成本極高，不適用於大樣本情境。
...
**答案：C**
**答案內容：類別變數具有高基數（High Cardinality）**
...
![PDF source page 218](images/source_pages/page_218.png)

![PDF source page 219](images/source_pages/page_219.png)

![PDF source page 220](images/source_pages/page_220.png)

![PDF source page 221](images/source_pages/page_221.png)

![PDF source page 222](images/source_pages/page_222.png)
...
**答案：C**
**答案內容：姓名、電子郵件地址、身分證字號**

**解析：** 「直接識別個人身份資訊（PII）」是指無需透過其他資訊輔助，單獨即可明確辨識出特定個人的資料。 （A）出生日期、郵遞區號和職業等資訊單獨無法直接識別特定個人，通常需要結合其他資訊才能達到識別效果，屬於準識別資訊（Quasi-Identifiers）。 （B）網站Cookie ID、IP 位址和地理定位資訊雖然屬於識別符號，且在特定情境下可被追溯或結合其他資訊後具有識別性，但其本身通常不被視為單獨可直接指向特定個人的PII。通常被歸類為間接識別資訊。 （C）姓名、電子郵件地址和身分證字號是典型的直接識別資訊，這些資訊單獨就能指向一個特定且唯一的人。 （D）網路行為模式、匿名化統計數據和加密後的密碼雜湊值均屬於非直接識別資訊。匿名化數據已去除識別性，加密雜湊值雖然源於個人密碼但已不可逆地處理，網路行為模式本身也不直接指向個人。
...
**答案：A**
**答案內容：對抗式去偏模型（Adversarial Fairness）**
...
**解析：** 公平性審查委員會匯集法務、技術、產品與倫理等部門，負責評估AI 系統的偏見風險，是企業治理AI 公平性的重要機制。
...
**答案：C**
**答案內容：知識蒸餾（Knowledge Distillation）**
