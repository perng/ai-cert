# 114 年第二次 AI 應用規劃師-中級能力鑑定：第三科「機器學習技術與應用」試題抽取

- 考試日期：114 年 11 月 08 日
- 試題公告日期：114 年 11 月 20 日
- 題數：50 題
...
某零售企業建立一個銷售預測模型，希望評估該模型在不同月份的新資料
...
在建立迴歸或分類模型時，若希望避免模型過度擬合（Overfitting），可
...
在訓練非線性模型時，若目標函數為非凸函數（Non-convex Function），
...
在執行 DBSCAN（Density-Based Spatial Clustering of Applications
...
某智慧製造公司開發一套影像辨識系統，用於自動檢測生產線上的瑕疵產
...
某智慧城市團隊開發一套交通監控系統，用於即時辨識路口監視器影像中
...
下列哪一種應用最適合採用長短期記憶網路（Long Short-Term Memory,
LSTM）模型？
(A)預測未來七天的電力需求變化趨勢；
(B)辨識監視影像中不同類別的物件；
(C)將大量顧客資料依相似特徵自動分群；
...
資訊增益（Information Gain）常用於衡量特徵對分類結果的不確定性貢
...
在建構以距離為基礎的機器學習模型（如KNN、SVM）時，下列哪一項資料
前處理方式最為關鍵？
...
相較於Grid Search，Random Search 在超參數調整上具備哪一項主要優
勢？
(A)可自動產生模型架構；
(B)可使用更大的訓練集；
(C)避免模型過擬合；
...
某智慧製造公司開發一套設備故障預測系統，利用感測器資料訓練深度神
...
下列哪一種AI 應用情境中，模型的可解釋性（Explainability）最為關
鍵？
...
在二元分類問題中，若精確率（Precision）為0.8，召回率（Recall）為
...
下列何者最能同時反映XGBoost（eXtreme Gradient Boosting）相較於傳
...
某醫療機構開發疾病早期偵測模型，正樣本（確診病例）僅佔 3%。在模型
...
某電子商務公司為開發商品評論情感分析模型，希望模型能捕捉評論中不
...
某語音辨識系統開發團隊採用 Transformer 架構，為了讓模型能同時理解
...
某電商平台希望預測顧客是否會購買特定商品。系統蒐集顧客的瀏覽紀
...
一家再生能源公司希望預測未來三個月太陽能發電量的波動範圍。由於氣
...
某房地產公司利用多元迴歸模型（Multiple Regression Model）預測房
...
某金融機構正在建立傳統信用評分卡模型，採用邏輯迴歸（Logistic
...
在防止監督式學習模型過擬合（Overfitting）時，下列哪一種策略不屬於
降低模型複雜度或限制學習能力的作法？
(A)採用L1 或L2 正則化；
...
某智慧製造團隊在開發瑕疵影像檢測模型時，發現使用線性激活函數
...
一家零售電商公司希望建立顧客流失預測模型，用以判斷哪些會員可能在
...
在工業設備故障預測專案中，模型訓練與超參數調整均依賴於一段歷史數
...
某情感分析模型在英文資料集上取得macro F1-score = 0.91。當該模型
...
某能源公司利用歷史氣象與用電資料，開發長期電力需求預測模型，採用
...
某電信公司開發客戶流失預測模型，使用大量顧客行為特徵，例如通話時
...
某資料科學團隊正在開發一個客戶相似度比對系統，用於計算所有客戶之
...
某醫療人工智慧團隊正在開發心臟病風險預測模型，資料量僅有150 筆，
...
某公司針對製程感測器資料進行主成分分析（PCA），經標準化與協方差矩
...
某銀行計畫與多家合作機構共同訓練一個AI 信用風險預測模型，為避免客
...
某跨銀行風控平台希望整合多家銀行的用戶行為資料，用於訓練信用風險
...
![Q38 figure 1](images/p10_img01.png)
...
![Q39 figure 1](images/p10_img02.png)
...
![Q40 figure 1](images/p10_img03.png)
...
考慮擲出骰子並採用Monte Carlo 方法估算條件機率，參考附圖程式碼。

事件 A：擲出偶數
事件 B：擲出大於3
請問下列何者為條件機率P(A∣B)的正確值？

![Q41 figure 1](images/p11_img01.png)
...
![Context for Questions 42-45](images/p11_img02.png)

## Question 42

**Answer: B**

在深度神經網路中，不同層的參數量（parameter count）差異極大。有些
...
在神經網路中，了解各層的運算量分佈，有助於模型壓縮與硬體加速的策
...
VGG16 層數深且結構規則，由多層卷積、池化及全連接層組成。了解各層
...
在實務應用中，我們常使用遷移學習(transfer learning)技巧，即載入預
...
(A)
![Q45 option A code](images/p14_img01.png)

(B)
![Q45 option B code](images/p14_img02.png)

(C)
![Q45 option C code](images/p14_img03.png)

(D)
![Q45 option D code](images/p14_img04.png)

## Context for Questions 46-47
...
![Context for Questions 46-47](images/p14_img05.png)

![Context for Questions 46-47](images/p15_img01.png)

## Question 46

**Answer: B**

假設研究人員已將含有雜訊的手寫數字影像存放在變數noisy 中。部分資
料經視覺化後的外觀如下：
...
![Q46 noisy generation code](images/p15_img02.png)

![Q46 noisy digit images](images/p15_img03.jpg)

![Q46 PCA denoising code](images/p15_img04.jpg)
...
研究人員在對digits 資料集進行分類時，決定使用KNN 並搭配交叉驗證來
...
![Q47 code option A](images/p15_img05.png)

![Q47 code option B](images/p16_img01.png)

![Q47 code option C](images/p16_img02.png)

![Q47 code option D](images/p16_img03.png)

請問哪幾組程式碼能正確使用KNN搭配交叉驗證，對digits 資料集進行訓
練並輸出準確率？
...
![Context for Questions 48-50](images/p16_img04.png)

![Context for Questions 48-50](images/p17_img01.png)

![Context for Questions 48-50](images/p17_img02.png)

![Context for Questions 48-50](images/p17_img03.png)
...
![Q48 standardization code 1](images/p17_img04.png)
...
![Q49 model summary/formula 1](images/p18_img01.png)

![Q49 model summary/formula 2](images/p18_img02.png)
...
![Q50 training plot 1](images/p18_img03.jpg)

![Q50 training plot 2](images/p18_img04.png)

![Q50 training plot 3](images/p19_img01.png)
