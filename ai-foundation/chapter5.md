...
*   **集成學習 (Ensemble)**：結合多個弱學習器。
    *   **Bagging (如 Random Forest)**：並行訓練 $\rightarrow$ 投票 $\rightarrow$ 降低變異 (Variance) $\rightarrow$ 抗過擬合。
    *   **Boosting (如 XGBoost)**：循序修正 $\rightarrow$ 加權 $\rightarrow$ 降低偏差 (Bias) $\rightarrow$ 提升準確度。
