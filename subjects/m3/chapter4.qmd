...
資料視覺化與報告情境題不只是選漂亮圖表，而是要讓決策者快速且正確理解資料。類別比較常用長條圖，趨勢變化常用折線圖，分佈與離群值可用箱型圖或直方圖，變數關係可用散佈圖或相關係數熱力圖。設計儀表板時應把最重要 KPI 放在第一視覺區，避免雙軸誤導、色彩過多、比例不一致或資料密度過低。好的報告會說明資料來源、限制、異常點、信心水準與建議行動，而不是只堆疊圖表。
...
請閱讀視覺化程式碼或資料輸出，判斷圖表設計與解讀方式。應能從欄位、圖表類型、座標軸、尺度、顏色與標籤判斷圖表是否能正確回答問題。長條圖適合類別比較，折線圖適合趨勢，散佈圖適合關係，箱型圖與直方圖適合分布與離群值。若程式使用雙軸、截斷座標或過多顏色，需能辨識可能造成誤導的地方，並提出更清楚的呈現方式。實務上還要注意圖表標題是否說明商業問題、單位是否一致、異常值是否標註，以及儀表板是否把關鍵指標放在最容易掃描的位置。
