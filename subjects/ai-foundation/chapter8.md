...
order: 8
label: chap-ai-foundation-chapter8
---
...
| **典型應用** | 垃圾郵件過濾、人臉辨識、房價預測（例如：判斷詐欺屬於鑑別式） | 寫作、繪圖、作曲、寫程式（例如：生成理賠報告屬於生成式） |
| **代表模型** | Logistic Regression, SVM, CNN (分類用) | VAE, GAN, Diffusion, Transformer (GPT) |

**更精確地說**：

*   **鑑別式 AI (Discriminative AI)** {#def-discriminative-ai}：學習輸入特徵 X 與標籤 Y 之間的條件關係，常用符號表示為 **P(Y|X)**。它關心的是「給我這筆資料，它屬於哪一類？」或「這筆資料的預測數值是多少？」。
*   **生成式 AI (Generative AI)** {#def-generative-ai}：學習資料本身的分佈，例如 **P(X)** 或 **P(X,Y)**，因此能從學到的分佈中產生新的資料樣本。它關心的是「這類資料長什麼樣？」以及「如何生成類似但不完全相同的新內容？」。

考題若出現「分類、辨識、風險評分、房價預測」通常指向鑑別式 AI；若出現「產生文字、圖片、音訊、影片、合成資料」通常指向生成式 AI。
...
*   **運作機制**：
    1.  **Encoder (編碼器)**：將圖片壓縮成兩個向量：平均值  𝝁 和標準差 **𝛔**。
        *   *類比*：把一張複雜的「披薩照片」壓縮成一張簡短的「食譜」（DNA）。
    2.  **Sampling (採樣)**：從常態分佈 N(μ, σ) 中抽樣出一個點 **z**。
...
### 影響生成模型效果的關鍵因素 {#sec-generative-model-quality-factors}

生成式模型的效果不是只由模型名稱決定，而是受到資料、架構、超參數與計算資源共同影響。

1.  **資料品質與多樣性**：
    *   高品質資料能讓模型學到穩定且可信的模式。
    *   多樣化資料能降低模型只會生成少數樣式的風險。
    *   若資料有偏見、錯誤或侵權內容，生成結果也可能繼承這些問題。
2.  **模型結構**：
    *   GAN 適合高品質生成，但訓練較不穩定。
    *   VAE 適合學習連續潛在空間與重建，但輸出可能較模糊。
    *   Diffusion 模型品質高且穩定，但生成速度通常較慢。
    *   Transformer 適合序列資料與語言生成，也可延伸到多模態生成。
3.  **超參數設定**：
    *   學習率、批次大小、訓練輪數、正則化強度等會影響收斂速度與品質。
    *   超參數不當可能導致訓練不穩、過擬合或生成品質不佳。
4.  **計算資源**：
    *   大型生成模型需要大量 GPU、記憶體、儲存與訓練時間。
    *   企業導入時必須評估訓練成本、推論成本、延遲與維運人力。

**考試重點**：若題目問「影響生成式 AI 效果的因素」，常見答案包含資料品質、模型結構、超參數與計算資源，而不是只看參數量或只看工具品牌。
...
2.  **微調 (Fine-tuning) - 職前訓練**： 讓「博學的怪人」變成「有用的助手」。
    ...
    *   **進階微調機制：強化微調 (Reinforcement Fine-tuning, RFT)** {#def-rft}：
        *   *強調重點*：其主要目的不僅是給予新知識，而是「透過 Reward (獎勵) 訊號調整模型的回應策略、行為偏好與優先順序」，使其輸出風格更符合企業品牌語氣與特定服務標準。
...
## 鑑別式 AI 與生成式 AI 的整合應用 {#sec-discriminative-generative-integration}

實務上，鑑別式 AI 與生成式 AI 不一定要分開使用。兩者整合後，可以同時具備「生成資料」與「判斷資料」的能力，常用於資料增強、模擬訓練、多模態互動與內容審核。

### 1. 數據增強與模型泛化 {#sec-gen-disc-data-augmentation}

生成式 AI 可以產生多樣化的合成資料，補足真實資料不足或不平衡的問題；鑑別式 AI 則使用這些資料進行分類、偵測或預測。

*   **醫療影像**：真實罕見病灶影像不足時，可用生成模型產生合成病灶影像，再訓練鑑別式模型辨識病灶。
*   **自動駕駛**：生成式 AI 模擬濃霧、夜間、雨天、施工道路等場景，讓鑑別式 AI 學習辨識行人、車輛與危險情境。
*   **金融風控**：生成式 AI 模擬極端市場或罕見詐欺交易，鑑別式 AI 再學習判斷風險。

重點不是用合成資料取代真實資料，而是增加資料多樣性，提升模型面對未見情境的泛化能力。

### 2. 多模態與即時回饋 {#sec-gen-disc-multimodal-feedback}

在多模態系統中，生成式 AI 可負責生成文字、語音或影像，鑑別式 AI 可負責辨識、分類與審核。

*   **智慧客服**：生成式 AI 產生回覆草稿，鑑別式 AI 檢查是否違反規範、是否含有敏感內容或是否與問題類型相符。
*   **內容生成與審核**：生成式 AI 產生廣告文案，鑑別式 AI 判斷是否有違規、偏見、侵權或品牌不一致問題。
*   **語音助理**：鑑別式 AI 先做語音辨識與意圖分類，生成式 AI 再產生自然語言回覆。

### 3. 整合應用的挑戰 {#sec-gen-disc-integration-challenges}

*   **生成模型訓練不穩定**：GAN 可能出現模式崩潰或梯度消失，可透過 WGAN 等改良方法提升穩定性。
*   **資料偏差與公平性**：生成式 AI 可能放大訓練資料偏見，進一步影響鑑別式模型的分類結果。
*   **合成資料品質控管**：低品質合成資料可能讓模型學到錯誤特徵，因此需要資料審核與評估。
*   **整合架構複雜**：生成、分類、審核與回饋流程需要清楚的系統設計、監控與版本管理。

**考試重點**：生成式 AI 支援鑑別式 AI 的典型價值是「資料增強、解決資料稀缺或不平衡、提升泛化能力」；鑑別式 AI 支援生成式 AI 的典型價值是「審核、分類、過濾與品質控管」。

## 本章總結與考點提示 {#chap-ai-foundation-chapter8-summary}
...
*   **優化技術**：
    ...
    *   **量化**：降低精度 (省資源)。
*   **整合應用**：
    *   **生成式 AI → 鑑別式 AI**：產生合成資料，解決資料稀缺與不平衡。
    *   **鑑別式 AI → 生成式 AI**：審核輸出、分類意圖、過濾不當內容。
