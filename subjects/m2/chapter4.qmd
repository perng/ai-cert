...
*   **博弈過程**：
    ...
    4.  警察也隨之提升鑑識能力。
    5.  這個過程不斷重複，直到偽鈔製造者的技術達到爐火純青，警察無法區分真假 D(G(z)) ≈ 0.5，達到**納什均衡 (Nash Equilibrium)**。

::: {.callout-tip}
## 納什均衡 (Nash Equilibrium)

在 GAN 的語境下，納什均衡指的是生成器和判別器都無法通過單獨改變自己的策略來獲得更好的結果。此時，生成器生成的數據分佈 P<sub>g</sub> 完美擬合真實數據分佈 P<sub>data</sub>，而判別器對於任何輸入（無論是真實的還是生成的）輸出的概率都是 0.5，即無法區分真假。
:::
...
*   $\mathbb{E}_{x \sim p_{data}(x)}$：表示對真實數據分佈 p<sub>data</sub>(x) 取期望值。即從真實數據集中採樣 **x**。
*   **D(x)**：判別器對真實數據 **x** 的輸出概率。理想情況下，**D(x)** 應接近 1。
*   **log(D(x))**：當 **D(x)** 接近 1 時，**log(D(x))** 接近 0（最大值）；當 **D(x)** 接近 0 時，**log(D(x))** 趨向負無窮（懲罰）。
*   $\mathbb{E}_{z \sim p_z(z)}$：表示對潛在空間雜訊分佈 p<sub>z</sub>(z) 取期望值。**z** 通常是隨機高斯雜訊。
...
1.  **前向過程 (Forward Process, **q**)**：
    *   這是一個**破壞**過程。
    *   逐步向真實圖像 x<sub>0</sub> 添加高斯雜訊，經過 **T** 步後，圖像變成純雜訊 x<sub>T</sub>（標準常態分佈）。
    *   公式：x<sub>t</sub> = √α<sub>t</sub> x<sub>t-1</sub> + √1-α<sub>t</sub> ε
2.  **反向過程 (Reverse Process, p<sub>θ</sub>)**：
    *   這是一個**生成**過程。
    *   訓練一個神經網路（通常是 U-Net）來預測每一步添加的雜訊 ε<sub>θ</sub>(x<sub>t</sub>, t)。
    *   從純雜訊 x<sub>T</sub> 開始，逐步減去預測的雜訊，最終恢復出清晰圖像 x<sub>0</sub>。

**公式術語解釋**：

*   x<sub>t</sub>：在時間步 **t** 的圖像（含有雜訊）。
*   α<sub>t</sub>：雜訊調度參數 (Noise Schedule)，控制每一步保留多少原圖訊息、加入多少雜訊。
*   **𝞮**：從標準常態分佈採樣的隨機雜訊。
*   ε<sub>θ</sub>：神經網路模型，輸入是有雜訊的圖 x<sub>t</sub> 和時間步 **t**，輸出是預測的雜訊。
...
[CLIP] 是由 OpenAI 開發的里程碑式模型，它成功地將**圖像**和**文本**映射到了同一個數學空間（向量空間）。
...
### 文本、語音與影像編輯生成 {.unnumbered}

生成式 AI 不只包含文字轉圖像，也涵蓋多種內容型態與編輯流程。

*   **文本生成**：GPT 類模型以自迴歸方式預測下一個 token，適合對話、摘要、長文撰寫、程式碼生成與客服回覆。
*   **語音生成 (Text-to-Speech, TTS)**：Tacotron、FastSpeech、VITS 等模型可由文字產生語音。VITS 結合變分推論與生成式架構，能端到端產生較自然的語音波形。
*   **影像編輯生成**：Stable Diffusion 類工具可進行 Inpainting、Outpainting、風格轉換與局部重繪，通常需要明確提示詞、遮罩與生成參數控制。

### 提示工程與生成品質控制 {.unnumbered}

提示工程 (Prompt Engineering) 是引導生成模型輸出符合需求內容的重要技能。

*   **提示元素**：主題、場景、風格、格式、長度、語氣、限制條件與負面提示。
*   **少樣本提示 (Few-shot Prompting)**：提供少量範例讓模型模仿輸出格式，可提升穩定性。
*   **生成風險**：模型可能產生幻覺、偏見、著作權疑慮或不自然細節。實務上需結合資料授權檢查、輸出審核、浮水印與人工覆核。
*   **多模態對齊問題**：文字編碼器與影像編碼器若未充分對齊，可能造成品牌顏色、文字標誌、手部姿勢或細節與提示不一致。


## 深度思考情境題 {#sec-scenario}

生成式 AI 與多模態情境題的核心是判斷「模型生成了什麼、依據什麼條件生成，以及風險如何控管」。GPT 類模型適合文本生成，擴散模型適合高品質圖像生成，CLIP 類模型可支援圖文對齊與零樣本分類。若輸出品牌標誌錯誤、手部不自然或與提示不一致，可能涉及多模態對齊與生成細節控制問題。實務導入時必須同時評估品質、多樣性、幻覺、著作權、內容安全、浮水印與人工審核流程。
