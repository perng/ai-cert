...
企業導入生成式 AI 應遵循嚴謹的流程，以確保成功落地並產生商業價值：
...
生成式 AI 帶來強大效能的同時，也伴隨著必須嚴格管理的風險：
...
## 部署與營運 (LLMOps) {#sec-llmops}

大型語言模型的營運 (LLMOps) 是 MLOps 的延伸，專注於 LLM 的生命週期管理：
...
### MLOps 部署與資料一致性補充 {.unnumbered}

*   **影子部署 (Shadow Deployment)**：新模型在背景接收真實流量並產生預測，但結果不回傳給使用者。它適合在不影響正式服務的情況下觀察新模型表現。
*   **特徵庫 (Feature Store)**：集中管理可重用特徵，確保訓練與線上推論使用同一套特徵定義，降低重工與不一致風險。
*   **訓練服務偏差 (Training-serving Skew)**：訓練時看到的資料、特徵處理或分布，和線上服務時不同，導致上線表現下降。
*   **共變量偏移 (Covariate Shift)**：輸入特徵分布改變，但標籤條件關係大致相同；例如使用者族群、季節或流量來源改變。
*   **人口穩定性指標 (Population Stability Index, PSI)**：常用於監控特徵或分數分布是否相對於基準資料發生明顯漂移。
