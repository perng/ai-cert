...
1.  **線性迴歸 (Linear Regression)**：
    ...
    *   **概念**：想像你在畫一張圖， 橫軸是房子的大小， 縱軸是房價。 你會發現點點大致分佈在一條線附近。 線性迴歸就是試著畫出這條「最佳直線」， 用來預測未來的趨勢。 
    *   **數學原理 (Friendly Math)**：
...
![線性迴歸](images/linear-regression.jpg){#fig-linear-regression}



2.  **邏輯迴歸 (Logistic Regression)**：
    *   **類型**：分類 (Classification)
    *   **概念**：雖然名字有「迴歸」， 但它其實是用來做「分類」的。 它像是一個開關， 判斷事情是「是」或「否」（0 或 1）。 
    *   **數學原理 (Friendly Math)**：
...
![邏輯迴歸](images/logistic-regression.jpg){#fig-logistic-regression}
